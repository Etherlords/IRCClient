package irc {
	...
	import irc.actions.connection.ConnectionAction;
	...
	import irc.loggers.IChatLoggerFactory;
	import irc.loggers.IChatLogger;
	import irc.loggers.IServerLogger;
	
	public class IRCClient extends EventDispatcher {
		private var _serverSetting:IRCServerSettingModel;
		...
		private var actionsMap:Vector.<IAction> = new Vector.<IAction>;
		
		public function IRCClient(loggerFactory:IChatLoggerFactory) {
			
			actionsMap[0] = new ConnectionAction();
			
			_serverSetting = (ServicesLocator.instance.getService(IRCSettingsService) as IRCSettingsService).settings;
			_loggerFactory = loggerFactory;
			_serverLogger = loggerFactory.createServerLogger(_serverSetting.host, _serverSetting.port);
			_channels = {};
			_names = {};
			_privateTalks = {};
			_registered = 0;
			_isRegistered = false;
			_reconnectionManager = new ReconnectionManager(_serverSetting.reconnectionAcceptableInterval, _serverSetting.reconnectionMaxCountWithinInterval);
		}
		...
		public function canOperateChannel(channelName:String):Boolean {
			return ((channelName in _channels) && (_me.nick in _channels[channelName].operators));
		}
		
		public function kick(channelName:String, nick:String, message:String = null):void {
			if (canOperateChannel(channelName))
				_connection.sendMessage(IRCCommands.KICK, [channelName, nick], message);
		}
		
		public function ban(channelName:String, nick:String):void {
			if (canOperateChannel(channelName))
				_connection.sendMessage(IRCCommands.MODE, [channelName, "+b", nick]);
		}
		
		public function kickAndBan(channelName:String, nick:String, message:String = null):void {
			if (canOperateChannel(channelName)) {
				_connection.sendMessage(IRCCommands.KICK, [channelName, nick], message);
				_connection.sendMessage(IRCCommands.MODE, [channelName, "+b", nick]);
			}
		...
		public function addVoice(channelName:String, nick:String):void {
			if (canOperateChannel(channelName))
				_connection.sendMessage(IRCCommands.MODE, [channelName, "+v", nick]);
		}
		...
		public function canSetTopic(channelName:String):Boolean {
			if (!(channelName in _channels))
				return false;
			return ((_channels[channelName].topicSetOnlyByOperator && canOperateChannel(channelName)) || !_channels[channelName].topicSetOnlyByOperator);
		}
		...
		public function canInvite(channelName:String, usernick:String):Boolean {
			if (!(channelName in _channels))
				return false;
			return ((_channels[channelName].isInviteOnly && canOperateChannel(channelName)) || !_channels[channelName].isInviteOnly);
		}
		
		public function invite(channelName:String, usernick:String):void {
			if (canInvite(channelName, usernick))
				_connection.sendMessage(IRCCommands.INVITE, [usernick, channelName]);
		}
		
		public function away(message:String):void {
			// XXX: should use 'MODE +a' instead of AWAY command?
			_connection.sendMessage(IRCCommands.AWAY, [], message);
		}
		
		public function part(channelName:String, message:String):void {
			_connection.sendMessage(IRCCommands.PART, [channelName], message);
		}
		...
		public function privmsg(target:String, message:String):void {
			...
			if (target.match(/^[#$!+]/) != null) {
				...
				if (!(target in _privateTalks)) {
					_privateTalks[target] = _loggerFactory.createPrivateTalk(target);
				}
				_privateTalks[target].sendPRIVMSG(_me.nick, message);
			}
			_connection.sendMessage(IRCCommands.PRIVMSG, [target], IRCUtil.escapeLowCTCP(message));
		}
		...
		public function notice(target:String, message:String):void {
			...
			_connection.sendMessage(IRCCommands.NOTICE, [target], IRCUtil.escapeLowCTCP(message));
		}
		...
		private function pong(msg:IRCMessage):void {
			var nick:String = (_me != null && _me.nick != null) ? _me.nick : "";
			_connection.sendMessage(IRCCommands.PONG, [msg.trailing]);
		}
		
		private function retryNick(m:IRCMessage):void {
			var unavailableNick:String = m.params[m.params.length - 1]
			_connection.sendMessage(IRCCommands.NICK, [unavailableNick + "_"]);
		}
		
		private function joinedChannel(m:IRCMessage):void {
			var channelName:String = m.trailing;
			_channels[channelName] = _loggerFactory.createChannel(channelName);
		...
		private function connectHandler(e:Event):void {
			if (_serverSetting.password != null && _serverSetting.password.length > 0)
				_connection.sendMessage(IRCCommands.PASS, [_serverSetting.password]);
			_connection.sendMessage(IRCCommands.NICK, [_serverSetting.nickname]);
			_connection.sendMessage(IRCCommands.USER, [_serverSetting.username, _serverSetting.usermode, "*"], _serverSetting.realname);
			dispatchEvent(e);
		}
